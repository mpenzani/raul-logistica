...
import { useState, useRef } from 'react'; // ✅ IMPORTS CORRIGIDOS
...
function App() {
  ...
  // 🎤 GRAVADOR 100% BULLETPROOF (SUBSTITUA TODO BLOCO)
...
const startSpeaking = async () => {
  ...
  try {
    ...
    // ✅ CODEC FALLBACK (resolve 95% crashes)
    ...
    recorder.onstop = () => {
      ...
      addXP();  // ✅ XP SYNC CORRETO, SEM RELOAD
      setIsRecording(false);
      chunks.length = 0;  // Reset leak
    ...
    recorder.start(250);  // Chunks pequenos = responsive
    setIsRecording(true);
  } catch (err: any) {
...
useEffect(() => () => stopSpeaking(), []);
  ...
  // 🔧 CARREGA LIÇÕES + PROGRESSO (MOVIDO CIMA)
  ...
  // PROGRESSO LOCALSTORAGE ✅ VARIÁVEIS DEFINIDAS
  const progressKey = `progress-${lead}`;
  ...
  {audioUrl && (
    <div style={{width: '100%', textAlign: 'center'}}>
      ...
      <button 
        ...
        style={{marginTop: '0.5rem', color: '#ef4444', fontSize: '0.9rem', background: 'none', border: 'none'}}
      >
        🔄 Nova Gravação
      </button>
    </div>
